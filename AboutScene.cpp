...
int About::Score = 0;
void About::setPhysicsWorld(PhysicsWorld* world) { m_world = world; }
//物理世界
Scene* About::createScene()
{
	...
	auto layer = About::create(scene->getPhysicsWorld());
	//创建物理世界场景
	scene->addChild(layer);
	return scene;
}

bool About::init(PhysicsWorld* world)
{
	if (!Layer::init())
	{
		return false;
	...
	this->schedule(schedule_selector(About::update), 0.1f, kRepeatForever, 0);
	//每0.1秒更新一次
	return true;
}

//预加载以及播放音乐
void About::preloadMusic()
{
...
void About::addBackground()
{
	//通过TMXTiledMap加载自己创作的砖块地图
	TMXTiledMap* tmx = TMXTiledMap::create("smile.tmx");
...
void About::addball()
{
	ball = Sprite::create("football.png");
...
void About::addContactListener()
{
	auto touchListener = EventListenerPhysicsContact::create();
	touchListener->onContactBegin = CC_CALLBACK_1(About::onConcactBegan, this);
	_eventDispatcher->addEventListenerWithFixedPriority(touchListener, 1);
}

void About::addKeyboardListener()
{
	auto keboardListener = EventListenerKeyboard::create();
	keboardListener->onKeyPressed = CC_CALLBACK_2(About::onKeyPressed, this);
	keboardListener->onKeyReleased = CC_CALLBACK_2(About::onKeyReleased, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(keboardListener, this);
}
//通过Tag碰撞检测
bool About::onConcactBegan(PhysicsContact& contact) {
...
//键盘按下
void About::onKeyPressed(EventKeyboard::KeyCode code, Event* event) {
	switch (code) {
...
//键盘放开
void About::onKeyReleased(EventKeyboard::KeyCode code, Event* event) {
	player->getPhysicsBody()->setVelocity(Vec2(0, 0));
}


About * About::create(PhysicsWorld * world) {
	About* pRet = new(std::nothrow) About();

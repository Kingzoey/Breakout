...
bool Breakout2::init(PhysicsWorld* world)
{
	...
	this->schedule(schedule_selector(Breakout2::update), 0.1f, kRepeatForever, 0);
	//每0.1秒更新一次
	return true;
}
...
void Breakout2::addBackground()
{
	...
	float windowswidth = mywinsize.width; //获取屏幕宽度
	float windowsheight= mywinsize.height;//获取屏幕高度
...
void Breakout2::addball()
{
	...
	ball->getPhysicsBody()->setVelocity(Vec2(0, 400));
...
bool Breakout2::onConcactBegan(PhysicsContact& contact) {
	...
	if (node1 && node2)
	{
		if ((node1->getTag() == 2 && node2->getTag() == 4) || (node1->getTag() == 4 && node2->getTag() == 2))
		{
			...
			{
				//下雪
				snow->setPosition(node2->getPosition());
				addChild(snow);
			...
			{
				//下雪
				snow->setPosition(node1->getPosition());
				addChild(snow);
